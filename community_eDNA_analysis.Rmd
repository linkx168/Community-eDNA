...
mn_map <- ggplot() +
  geom_sf(data = mn_sf, fill = "white", color = "gray50") +
  geom_point(data = lakes_proj, aes(x = longitude, y = latitude), size = 2, color = "lightblue") +
  labs(x = NULL,
       y = NULL) +
  geom_segment(
    data = lakes_proj,
...
inal_map <- ggdraw() +
  draw_plot(mn_map) +
  draw_plot(us_inset, x = .7, y = 0.8, width = 0.2, height = 0.2)
inal_map
ggsave("sf_icon_map_inset_small.jpg",  height = 4, width = 4, dpi = 600)
```

# post-sampling survey results 
```{r}
survey <- read_csv("post_sampling_survey.csv")
glimpse(survey)

rating_questions <- survey %>%
  select(`Overall, how effective do you feel you were at eDNA sampling?`,
         `How easily were you able to find and access a sampling location?`,
         `How easy was it to collect the water sample?`,
         `How useful were the included written instructions?`,
         `How useful were the instructional videos?`,
         `How useful were the pre-sampling webinars?`,
         `Was the sampling equipment easy to use?`)

percent_4plus <- rating_questions %>%
  pivot_longer(everything(), names_to = "question", values_to = "rating") %>%
  group_by(question) %>%
  summarize(
    n = sum(!is.na(rating)),
    above_4 = sum(rating >= 4, na.rm = T),
    pct_4plus = round(above_4/n * 100, 0),
    mean_rating = mean(rating, na.rm = TRUE),
    sd_rating = sd(rating, na.rm = TRUE)
  )

labels <- c(
  "Overall, how effective do you feel you were at eDNA sampling?" = "Effective at sampling",
  "How easily were you able to find and access a sampling location?" = "Finding location",
  "How easy was it to collect the water sample?" = "Ease of sampling",
  "How useful were the included written instructions?" = "Written instructions",
  "How useful were the instructional videos?" = "Instructional videos",
  "How useful were the pre-sampling webinars?" = "Webinars",
  "Was the sampling equipment easy to use?" = "Equipment ease"
)

ratings <- ggplot(percent_4plus, aes(x = reorder(question, -pct_4plus), y = pct_4plus)) +
  geom_col() +
  scale_x_discrete(labels = labels) +
  labs(x = NULL, y = NULL) +
  theme_bw() +
  theme(
    axis.text.x = element_text(size = 10, angle = 45, hjust = 1),  # tilt labels
    axis.text.y = element_text(size = 11),
    axis.title = element_text(size = 12)
  )
ratings
rm(rating_questions, percent_4plus, labels)

survey %>% 
  filter(!is.na(`Approximately how long did it take you to collect and filter your sample (not including travel time):`)) %>% 
  rename(`How Long Did It Take You to Sample?` =
           `Approximately how long did it take you to collect and filter your sample (not including travel time):`) %>% 
  summarise(n = n(),
            n_15 = sum(str_detect(`How Long Did It Take You to Sample?`, "Less")),
            n_30 = sum(str_detect(`How Long Did It Take You to Sample?`, "15 - 30")),
            n_60 = sum(str_detect(`How Long Did It Take You to Sample?`, "60"))) %>% 
  mutate(under_30 = (n_15+n_30)/n)
...
time_pct <- survey %>% 
  ...
  rename(`time` =
           `Approximately how long did it take you to collect and filter your sample (not including travel time):`) %>% 
  mutate(time = case_when(time == "Less than 15 minutes" ~ "< 15 mintues",
                          TRUE ~ time)) %>% 
  count(`time`) %>% 
  mutate(pct = n / sum(n) *100) %>% 
  ggplot() +
  geom_col(aes(x = reorder(time, -pct), y = pct)) +
  labs(y = NULL,
       x = NULL) +
  theme_bw() +
...
combined <- ratings + time_pct
combined
#ggsave("post_sampling_survey.jpg", height = 5, width = 7, dpi = 600)
```
